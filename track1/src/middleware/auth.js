...
function authMiddleware(req, res, next) {
  ...
  try {
    ...
    if (err.name === 'TokenExpiredError') {
      return res.status(401).json({ error: 'TOKEN_EXPIRED', message: 'JWT has expired.', traceId });
    }
    return res.status(401).json({ error: 'UNAUTHORIZED', message: 'Invalid JWT.', traceId });
  }
}

/**
 * Enforce that the authenticated user can only access their own data.
